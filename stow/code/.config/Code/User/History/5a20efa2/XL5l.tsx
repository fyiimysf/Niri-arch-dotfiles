...
export default function TerminalBirthday() {
  ...
  const initializeMatrixStreaks = useCallback(() => {
    ...
    const isMobile = window.innerWidth < 768
    const streakCount = isMobile ? 25 : 40
          ...
          {matrixStreaks.map((streak) =>
            streak.chars.map((char, charIndex) => (
              <div
                ...
                className="absolute w-screen font-mono pointer-events-none text-green-400"
                style={{
                  left: `${streak.x}px`,
                  top: `${char.y}px`,
      ...
      <div className="absolute inset-0 top-6 sm:top-8 z-10">
        {matrixChars
          ...
          .map((char, index) => (
            <div
              key={index}
              className="absolute font-mono pointer-events-none transition-all duration-10"
              style={{
                left: `${char.x}px`,
                top: `${char.y}px`,
                opacity: char.opacity,
