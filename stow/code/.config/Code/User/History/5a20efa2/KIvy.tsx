  ...
  return (
    <div className="fixed inset-0 bg-black text-green-400 overflow-hidden font-mono border-2 border-green-600 select-none">
      <div className="bg-green-900/50 px-2 sm:px-3 py-1 text-xs sm:text-sm text-green-300 border-b border-green-600 flex items-center justify-between shadow-lg shadow-green-400/20">
        <span className="font-bold text-xs sm:text-sm truncate">aro@b3lla:~/terminal_protocol$</span>
      ...
      <div className="absolute inset-0 top-5 sm:top-15 z-10">
        {matrixChars
          .filter((char) => char.isTarget)
          .map((char, index) => (
            <div
              key={index}
              className="absolute font-mono pointer-events-none transition-all duration-2000"
              style={{
                left: `${char.x}px`,
                top: `${char.y}px`,
                opacity: char.opacity,
      ...
      {showStartButton && (
        <div className="absolute inset-0 backdrop-blur-sm bg-black/50 flex items-center justify-center z-20 p-4">
          <div className="text-center">
            <div className="mb-6 sm:mb-8 text-green-300 text-md sm:text-xl animate-pulse font-bold tracking-wider px-4">
              ◆ BOOT THE MATRIX ◆
            </div>
            <Button
              onClick={startAnimation}
              className="bg-green-600 hover:bg-green-500 text-black font-mono text-base sm:text-lg px-4 sm:px-8 py-2 sm:py-4 border-2 border-green-400 shadow-lg shadow-green-400/50 transition-all duration-200 hover:shadow-green-400/70 hover:scale-105 touch-manipulation"
            >
              ▶ JACK IN
            </Button>
          </div>
